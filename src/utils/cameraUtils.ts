...
export const calculateFraming = (
    ...
    // Cap the vertical framing at 4.0m (which is about 26.66 physics units given 1u = 0.15m)
    // After this point, the camera stops zooming out and stays focused on the lower part of the tower.
    const heightForCalc = Math.min(Math.max(totalHeight, 0.25), 26.66);
    ...
    const targetYFactor = viewAngle === 'isometric' ? 0.28 : 0.30;
